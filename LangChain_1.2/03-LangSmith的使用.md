...
![LangSmith主界面](assets/01-langsmith-main.png)
...
![LangSmith注册或登录页面](assets/02-langsmith-signup.png)

**步骤2：自由选择注册或登录方式**

**步骤3：登录成功**

![LangSmith登录成功页面](assets/03-langsmith-login-success.png)

### 2.2 获取API_KEY

**步骤1：打开设置**

![打开LangSmith设置](assets/04-open-settings.png)

**步骤2：创建API_KEY**

![API Keys页面](assets/05-api-keys-page.png)

![创建API Key](assets/06-create-api-key.png)

**步骤3：保存KEY**

![复制新建的API Key](assets/07-copy-api-key.png)

> **注意**：点击 `copy` 将 API_KEY 保存到剪贴板并关闭弹窗，API_KEY 只在上述窗口出现一次，关闭弹窗后回到设置页面，此后就无法在官网查看 API_KEY 的内容了，请妥善保存。

![API Key保存后的列表](assets/08-api-key-saved.png)

**步骤4（可选）：按需删除**

如果需要删除 KEY，点击右侧图标即可。

![删除API Key](assets/09-delete-api-key.png)

![确认删除API Key](assets/10-confirm-delete-api-key.png)
...
![Tracing项目列表](assets/11-tracing-project.png)
...
![运行指标详情](assets/12-run-details.png)

此处列出了详细的运行指标，点击某次运行记录，可以查看更详细的信息，自行探索。

**步骤4：查看运行报表**

![选择Monitoring项目](assets/13-monitoring-project.png)

此处提供了大量指标的报表。

![Monitoring指标报表](assets/14-monitoring-traces.png)

点击上述标签或下滑页面可以切换指标。

![LLM Calls与Cost Tokens报表](assets/15-monitoring-llm-calls.png)
